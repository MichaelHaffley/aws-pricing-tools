...
FORECAST_PERIOD_EVERY_FIVE_MINUTES = 'every_five_minutes'
DEFAULT_FORECAST_PERIOD = FORECAST_PERIOD_MONTHLY
HOURS_DICT = {FORECAST_PERIOD_MONTHLY:720.0, FORECAST_PERIOD_HOURLY:1.0, FORECAST_PERIOD_EVERY_FIVE_MINUTES:1.0/12.0}
...
def handler(event, context):
    ...
    try:
        ...
        labs = get_key_values(tagkey)
        for tagvalue in labs:
            try:
                ...
                totalCost = totalCost / (HOURS_DICT[FORECAST_PERIOD_MONTHLY] * 12.0)
                result['pricingRecords'] = pricing_records
                result['totalCost'] = round(totalCost,4)
                result['forecastPeriod']=FORECAST_PERIOD_EVERY_FIVE_MINUTES
...
def get_key_values(tagkey):
    response = tagsclient.get_tag_values(PaginationToken='', Key=tagkey)
